...
func SendTransactions(client *ethclient.Client, addr_to string, priv_k string, ch chan<-string, tps int, test_size int, check_txs bool, heavy_io bool)  {
	...
	log.Println("Sender:  ",common.Bytes2Hex(fromAddress[:])," balance ", balance, "nonce ", nonce)
	...
	for i:=0; i < test_size; i++ {
		...
		log.Println("signed tx", signedTx.Hash().Hex())
		if err != nil {
			log.Fatal(err)
		}
...
func GetTransactionsBlockCount(client *ethclient.Client, ch chan string)  {
	...
	for {
		...
		case <-ticker.C:
			//ch <- proc_txs
			ch <- fmt.Sprint("Transactions in the block #", blockhash.NumberU64(), " :", proc_txs, "\n",
